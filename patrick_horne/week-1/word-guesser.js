...
function guessLetter(guessedLetter) {
  ...
  if (hangmanState >= 6) {
    console.log('You\'re dead. RIP.');
  } else if (rewardMultiplier) {
    let winningsTempValue = Math.random() * 100;
    reward += winningsTempValue;
    console.log(`You won an additional $${winningsTempValue.toFixed(2)}`);
  } else {
    reward -= 20;
  }
}
